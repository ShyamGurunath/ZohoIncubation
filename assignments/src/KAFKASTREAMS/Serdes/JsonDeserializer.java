...
public class JsonDeserializer<T> implements Deserializer<T> {
    ...
    public void configure(Map<String, ?> props, boolean isKey) {
        if (isKey)
            ...
            //System.out.println(props.get(VALUE_CLASS_NAME_CONFIG));
            try {

                className = props.get(VALUE_CLASS_NAME_CONFIG) instanceof String ?
                        (Class<T>) Class.forName((String) props.get(VALUE_CLASS_NAME_CONFIG)) :
                        (Class<T>) props.get(VALUE_CLASS_NAME_CONFIG);
    ...
    public T deserialize(String topic, byte[] data) {
        ...
        try {
            return objectMapper.readValue(data, (TypeReference<T>) new TypeReference<User>() {
            });
        } catch (Exception e) {
